...
* `Mesh::OWENSFEA:Mesh`: mesh without deformation. Storing a copy here to clean up the code a bit.
* `Ort::OWENSFEA:Ort`: orientations of all elements without deformation.  Storing here since we don't have a good way of passing this info during simulation
...
Note on angles
    The OWENSFEA mesh in OWENS uses +X as the blade/strut long axis.  In AeroDyn, the blade axis is +Z.  So for transforming the
    blades from OWENSFEA mesh coordinates, first rotate by -90 degrees about Y, then do the 3,2,1 coordinate transforms with
...
$(length(airfoil_filenames))                      NumAFfiles  - Number of airfoil files used (-)
$AF_str
